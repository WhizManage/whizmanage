...
const AddLabelsItem = ({ columnName, index, handleRowChange }) => {
  ...
  useEffect(() => {
    ...
    } else if (window.listOrders.length > 0) { // מחכה שהנתונים יגיעו
      setExistingItems(window.listOrders);
    }
  }, [window.listOrders]); // יופעל שוב כש`ordersData` יתעדכן
  ...
  const addNewItem = async (item) => {
    ...
    await postApi(
      `${window.siteUrl}/wp-json/wc/v3/orders/${objects}`,
      itemData
    )
      .then((res) => {
