...
import { Check, Pencil, Plus, TextSearch, Undo2, XCircle, CheckSquare, X } from "lucide-react";
...
const SelectGlobalOptions = ({
  ...
  const isAllSelected =
    itemsExist.length > 0 &&
    selectedAttributes[index]?.options.length === itemsExist.length;
  ...
  const handleSelectAll = () => {
    const updatedSelectedAttributes = selectedAttributes.map((attr, idx) => {
      if (idx === index) {
        return {
          ...attr,
          options: [...itemsExist], // כל האופציות
        };
      }
      return attr;
    });
  ...
  const addNewItem = async (item) => {
    ...
    try {
      ...
      if (!product[key]) {
        // If the key doesn't exist, add it with an initial value of res?.data
        product[key] = [res?.data];
      } else {
        // If the key exists, add the value to the array
        product[key].push(res?.data);
      }
      const data = {
        update: [
          {
      ...
      <PopoverContent className="p-0 dark:bg-slate-800" align="start">
        <Command className="dark:bg-slate-800">
          ...
          <CommandList>
            ...
            <CommandGroup>
              <div className="flex justify-between items-center gap-2 px-3 pt-2">
                <div className="flex gap-2">
                  {itemsExist.length > 0 && !isAllSelected && (
                    <Button
                      variant="ghost"
                      size="sm"
                ...
                <span className="text-xs p-0">
                  {selectedAttributes[index]?.options.length || 0} {__("Selected", "whizmanage")}
                </span>
              </div>
            ...
            <CommandGroup
              heading={
                itemsExist && itemsExist.length > 0 ? __(`Options exist`, "whizmanage") : ""
              }
            >
              {isLoading ? (
