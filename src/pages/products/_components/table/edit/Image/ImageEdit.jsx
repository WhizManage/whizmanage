...
import { Check, CheckCircle, ImagePlus, Trash2, Copy } from "lucide-react";
...
import ModalFooterPagination from "./ModalFooterPagination";
...
const ImageEdit = ({ row, setImage }) => {
  ...
  const handleSave = () => {
    if (row) {
      if (row.depth == 0) row.original.images[0] = selectedImage;
      else row.original.image = selectedImage;
    } else {
  ...
  const hasImage =
    row?.depth == 0 ? row?.original?.images?.[0]?.src : row?.original?.image?.src;
                          ...
                          >
                            <CardHeader className="absolute z-10 top-0 flex gap-1 !items-end justify-end">
                              ...
                              <Button
                                ...
                                size="sm"
                                isIconOnly
                                onClick={(event) => deleteImage(image.id, event)}
