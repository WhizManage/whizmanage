// עדכון DataTable.jsx עבור טבלת הקופונים
import {
  flexRender,
  getCoreRowModel,
  getExpandedRowModel,
...
import { useTheme } from "next-themes"; // הוספת הוק useTheme
...
import { couponFilters } from "@/data/defaultFilters";
...
export function DataTable({
  ...
  const [enableFilters, setEnableFilters] = useState(couponFilters);
  ...
  const [isResizing, setIsResizing] = useState(false); // הוספת state לעקיבה אחר מצב שינוי גודל
  ...
  const [columnPinning, setColumnPinning] = useState(() => {
    ...
    )?.reservedData || { left: ["select"], right: [] };
    return data;
  });
  ...
  useEffect(() => {
    ...
    const time = setTimeout(async () => {
      const msg = { name: "coupons_columns_width", reservedData: tableColumns };
      const url = window.siteUrl + "/wp-json/whizmanage/v1/columns/" + msg.name;
                  ...
                  {table.getHeaderGroups().map((headerGroup) => (
                    <TableRow key={headerGroup.id}>
                      ...
                      {!editAll && (
                        ...
                        >
                          <HeaderShadowEffect pinSide={isRTL ? "left" : "right"} />
                      ...
                      <div className="flex items-center gap-4 text-left bg-white dark:bg-slate-800 p-5 rounded-lg shadow-md border-l-4 border-l-fuchsia-600">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-gray-800 dark:text-white">
                            {__("No coupons found", "whizmanage")}
                          </h3>
