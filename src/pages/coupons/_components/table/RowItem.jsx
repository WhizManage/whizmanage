...
function RowItem({
  ...
  const updateCoupon = async (row) => {
    ...
    const url = `${window.siteUrl}/wp-json/wc/v3/coupons/${_bodyData.id}`;

    if (_bodyData?.usage_limit == "") _bodyData.usage_limit = null;
    if (_bodyData?.usage_limit_per_user == "") _bodyData.usage_limit_per_user = null;
    if (_bodyData?.limit_usage_to_x_items == "") _bodyData.limit_usage_to_x_items = null;
    ...
    await putApi(url, _bodyData)
      .then((data) => {
        ...
        showToast("success", __("The coupon has been updated successfully", "whizmanage"));
        if (isEditing) {
          toggleEdit(row);
        }
            ...
            >
              {isTrash ? (
                <ActionButton
                  ...
                  tooltip={__("Restore coupon", "whizmanage")}
                  isLoading={isLoading}
                ...
                isEditing ? (
                  ...
                  <ActionButton
                    ...
                    tooltip={__("Edit coupon", "whizmanage")}
                    isLoading={isLoading}
                    loadingIcon={<RefreshCcw className="size-4 animate-spin" />}
                  />
                )
