...
function DisplayColumns({ table, plusButton, metaData, allMetaData }) {
  ...
  const handleUpdateView = async () => {
    ...
    const msg = { name: "coupons_visible_columns", reservedData: columnsVisibility };
  ...
  const handleDragEnd = async (event) => {
    ...
    if (active && over && active.id !== over.id) {
      ...
      const msg = { name: "coupons_columns_order", reservedData: updatedColumnOrder };
      const url = window.siteUrl + "/wp-json/whizmanage/v1/columns/" + msg.name;
  ...
  const resetCouponsTable = async () => {
    ...
    if (isConfirmed) {
      const widthMsg = { name: "coupons_columns_width", reservedData: initialColumnSizing };
      const widthUrl = window.siteUrl + "/wp-json/whizmanage/v1/columns/" + widthMsg.name;

      const orderMsg = { name: "coupons_columns_order", reservedData: initialColumnOrder };
      const orderUrl = window.siteUrl + "/wp-json/whizmanage/v1/columns/" + orderMsg.name;

      const visibleColumnsMsg = { name: "coupons_visible_columns", reservedData: initialReservedData };
      const visibleColumnsUrl = window.siteUrl + "/wp-json/whizmanage/v1/columns/" + visibleColumnsMsg.name;

      const pinnedColumnsMsg = { name: "coupons_pinned_columns", reservedData: { left: ["select", "expand"], right: [] } }
      ...
      try {
        ...
        window.getWhizmanage = (window.getWhizmanage || []).map(c => c.name === "coupons_pinned_columns" ? { ...c, reservedData: pinnedColumnsMsg.reservedData } : c);
              ...
              <CommandList className="scrollbar-whiz">
                ...
                >
                  {metaData
                    ...
                    : table.getState().columnOrder.filter(c => c != "select").map((columnId) => {
                      const column = table.getColumn(columnId);
                      return (
                        column && (
                          <SortableColumn
