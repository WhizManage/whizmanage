...
const TabItem = ({ active, label, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`relative px-2 py-0.5 text-sm font-medium transition-colors duration-200 ${
        active
          ? "text-fuchsia-600"
...
const DateFilterTabs = ({ onDateRangeChange, columnFilters }) => {
  ...
  useEffect(() => {
    const hasDateFrom = columnFilters.some((f) => f.id === "date_from" && f.value);
    const hasDateTo = columnFilters.some((f) => f.id === "date_to" && f.value);

    // If date filters were cleared externally, reset local state
    if (!hasDateFrom && !hasDateTo && activeTab !== "custom") {
      setActiveTab("custom");
      setCustomStartDate(null);
      setCustomEndDate(null);
      lastAppliedDates.current = { startDate: null, endDate: null };
    }
  }, [columnFilters]);
  ...
  return (
    <div className="relative overflow-hidden px-2">
      <div className="flex space-x-1 rtl:space-x-reverse overflow-x-auto overflow-y-hidden scrollbar-none">
        <TabItem
