// src/layout/editor/components/TableDialog.jsx
...
const TableDialog = ({ isOpen, onClose, onInsert, row, currency = "" }) => {
  ...
  const handleInsert = () => {
    ...
    if (hasHeader) {
      tableHtml += "<thead><tr>";
      const headers = headerLabels.length
        ? headerLabels
        : Array.from({ length: cols }, (_, j) => `${__("Header", "whizmanage")} ${j + 1}`);
  ...
  const generateTableVariations = () => {
    // Check if row exists
    if (!row) {
      alert(t("No product data available"));
      return;
    }

    // Check if product is variable type
    if (row.type !== "variable") {
      alert(t("This feature is only available for variable products"));
      return;
    }
    ...
    const variationAttributes = attributes.filter((a) => a?.variation === true);

    if (!variationAttributes.length) {
      alert(t("No variation attributes found"));
      return;
    }

    if (!Array.isArray(row?.subRows) || row.subRows.length === 0) {
      alert(t("No variations found for this product"));
      return;
    }

    const headers = variationAttributes.map((a) =>
      (a?.name ?? "").replace(/^./, (c) => c.toUpperCase())
    );
    headers.push(__("Price", "whizmanage"));
    ...
    if (!rowsWithPrice.length) {
      alert(t("No variation data to display"));
      return;
    }
  ...
  return (
    <Modal
      ...
      classNames={{
        backdrop:
          "bg-gradient-to-t from-zinc-900 to-zinc-900/10 backdrop-opacity-20 z-[10001]",
          ...
          <>
            <ModalHeader className="flex gap-3 justify-center items-center">
              ...
              <h2 className="text-xl font-semibold dark:text-slate-200">
                {__("Insert Table", "whizmanage")}
              </h2>
            </ModalHeader>
            <Button
              variant="outline"
              size="sm"
              onClick={generateTableVariations}
              className="flex mx-7 items-center"
            >
            ...
            <ModalBody>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="flex items-center">
                      <Input
                        ...
                        value={width}
                        onChange={(e) =>
                          setWidth(parseInt(e.target.value, 10) || 100)
                        }
                        className="flex-1"
                      />
                ...
                <div className="overflow-auto scrollbar-whiz border rounded p-2 dark:border-slate-600">
                      ...
                      <thead>
                        <tr>
                          {(headerLabels.length > 0
                            ? headerLabels.slice(0, 5)
                            : Array.from({ length: Math.min(cols, 5) }).map(
                                (_, j) => `${__("H", "whizmanage")}${j + 1}`
                              )
                          ).map((label, j) => (
                            <th
                              key={`header-${j}`}
            ...
            <ModalFooter>
              <Button
                variant="outline"
                onClick={onCloseFunc}
                aria-label={__("Cancel", "whizmanage")}
              >
                {__("Cancel", "whizmanage")}
              </Button>
              <Button onClick={handleInsert} aria-label={__("Insert Table", "whizmanage")}>
                {__("Insert Table", "whizmanage")}
              </Button>
            </ModalFooter>
          </>
