// src/layout/editor/components/TableSelector.jsx
...
const TableSelector = ({ editorRef, execCommand, onContentChange }) => {
  ...
  const ensureCellSelected = (table) => {
    ...
    if (!t) return null;
    let cell =
      t.querySelector("td.selected, th.selected") || t.querySelector("td, th");

    // (אופציונלי) לנקות תא קודם בתוך אותה טבלה
    if (
      cell &&
      selectedCell &&
      selectedCell !== cell &&
      selectedCell.isConnected &&
      selectedCell.closest("table") === t
    ) {
      try {
  ...
  const positionToolbar = (tableEl) => {
    ...
    try {
      ...
      const tbW = toolbarRef.current?.offsetWidth ?? 200;
      const maxLeft = Math.max(0, window.innerWidth - tbW - 8);
  ...
  const getActiveRefs = () => {
    const doc = editorRef?.current?.contentDocument;
    let table = selectedTable?.isConnected
      ? selectedTable
      : doc?.querySelector("table.selected, table.umedia-selected") ||
        doc?.querySelector("table");
    if (table !== selectedTable) setSelectedTable(table || null);

    const cell = table ? ensureCellSelected(table) : null;
  ...
  useEffect(() => {
    ...
    if (!doc) return;
    const onStart = () => {
      if (selectedTable && showToolbar) positionToolbar(selectedTable);
    };
    const onEnd = () => {
      if (selectedTable && showToolbar) {
        positionToolbar(selectedTable);
        startFollow();
      }
    };
  ...
  const hasHeader = () =>
    selectedTable && selectedTable.querySelector("thead") !== null;
  ...
  const makeTableFullWidth = () => {
    const { table } = getActiveRefs();
    if (!table) return;

    try {
      setTableWidthPct(table, 100);
      resetUniversalOverlay("table-full-width");
      updateContent();
    } catch (e) {
      console.error("makeTableFullWidth failed", e);
    }
  };
  ...
  const insertColumnLeft = () => {
    ...
    Array.from(table.rows).forEach((row, rowIndex) => {
      ...
      const baseStyle =
        ...
        "; padding:8px; min-width:80px; min-height:30px;";
  ...
  const deleteRow = () => {
    ...
    if (table && table.rows.length > 1) {
      ...
      alert(__("Cannot delete the last row", "whizmanage"));
    }
  };
  ...
  const deleteColumn = () => {
    ...
    if (table.rows[0] && table.rows[0].cells.length > 1) {
      ...
      alert(__("Cannot delete the last column", "whizmanage"));
    }
  };
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      let _cell = cell;
      if (!_cell && table) {
        _cell = table.querySelector("td, th");
      }

      if (
        e.target.closest?.(".umedia-toolbar") ||
        e.target.closest?.(".table-toolbar")
      )
        return;

      doc.querySelectorAll("table.selected").forEach((t) => {
        if (t !== table) t.classList.remove("selected");
      });
      doc
        .querySelectorAll("td.selected, th.selected")
      ...
      if (table) {
        ...
        try {
          requestAnimationFrame(() => positionToolbar(table));
        } catch {}
        try {
    ...
    const onScroll = () => {
      if (selectedTable && !isDialogOpen) positionToolbar(selectedTable);
    };
    const onWinResize = () => {
      if (selectedTable) positionToolbar(selectedTable);
    };
    const onOuterScroll = () => {
      if (selectedTable) positionToolbar(selectedTable);
    };
    ...
    if (outerScrollEl)
      outerScrollEl.addEventListener("scroll", onOuterScroll, {
        passive: true,
      });
    ...
    const onTableAction = (e) => {
      ...
      switch (action) {
        ...
        case "deleteTable":
          ...
          break;
        case "fullWidth":
          makeTableFullWidth && makeTableFullWidth();
          break;
        default:
          break;
      }
    ...
    doc.addEventListener("umedia:table-action", onTableAction);
    ...
    return () => {
      ...
      doc.removeEventListener("umedia:table-action", onTableAction);
      if (style.parentNode)
        try {
  ...
  const ensureInitialWidth = (table) => {
    ...
    if (!hasExplicitWidth) {
      ...
      try {
        const doc = editorRef?.current?.contentDocument;
        doc?.dispatchEvent(
          new CustomEven__("umedia:reset", {
            bubbles: true,
            composed: true,
            detail: { reason: "table-auto-100" },
          })
        );
      } catch {}
      updateContent();
    ...
    return createPortal(
      ...
      >
        ...
        <Button
          ...
          variant="ghost"
          className="h-7 w-7 p-0 rounded-md"
          onClick={makeTableFullWidth}
          title={__("Full width", "whizmanage")}
        >
        ...
        <Button
          ...
          className="h-7 w-7 p-0 rounded-md"
          onClick={deleteTable}
          title={__("Delete table", "whizmanage")}
        >
        ...
        {selectedCell && (
          <>
            ...
            <div className="relative">
              ...
              {showRowOptions && (
                <div
                  className="absolute top-full left-0 mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md shadow-lg z-10 min-w-32"
                  style={{ zIndex: 100000 }}
                >
                  <Button
                    size="xs"
                    variant="ghost"
                    className="w-full justify-start px-3 py-1.5 text-xs rounded-none"
                  ...
                  <Button
                    size="xs"
                    variant="ghost"
                    className="w-full justify-start px-3 py-1.5 text-xs rounded-none text-red-500"
                    onClick={deleteRow}
                  >
