// src/layout/editor/components/EditingArea.jsx
...
const EditingArea = ({
  ...
  return (
    <div
      className={`flex-grow overflow-auto scrollbar-whiz relative ${compact ? "dark:!bg-slate-700" : "dark:!bg-slate-900"}`}
      style={{ height }}
    >
      {isPreview ? (
        ...
        <iframe
          src="about:blank"
          ref={editorRef}
          className={`w-full h-full border-0 scrollbar-whiz ${compact ? "dark:!bg-slate-700" : "dark:!bg-slate-900"}`}
          title="HTML Editor"
          onLoad={() => {
            const doc = getDoc(editorRef);
