// src/layout/editor/components/UniversalMediaSelector.jsx
import { useDebounceFn } from "@/components/table/hooks/useDebounce";
import { useTheme } from "@/layout/ThemeProvider";
import { Button } from "@components/ui/button";
import { Maximize, PlusCircle, Settings, Trash2 } from "lucide-react";
...
export default function UniversalMediaSelector({ editorRef, onContentChange }) {
  ...
  const debouncedOnChange = useDebounceFn((html) => onContentChange?.(html), {
    wait: 120,
    maxWait: 500,
  });
  ...
  // בחירה – כולל TABLE (הפעם גם לטבלה יש טולבר יוניברסלי)
  useEffect(() => {
    const iframe = editorRef?.current;
    const doc = iframe?.contentDocument;
    if (!doc) return;
    ...
    const onClick = (e) => {
      ...
      if (table) {
        // לא עוצרים בועות -> TableSelector עדיין יקבל את הקליק (לבחירת תא וכו')
        clearAllSelections(doc);
        table.classList.add("umedia-selected");
        setSelectedEl(table);
        positionToolbarOver(table, iframe, setToolbarPos, toolbarRef); // הטולבר מעל הטבלה
        return;
      }
    ...
    if (outerScrollEl)
      outerScrollEl.addEventListener("scroll", onScroll, { passive: true });
  ...
  // עטיפה/אוברליי/ריסייז – לכל מדיה, כולל TABLE
  useEffect(() => {
    ...
    try {
      ...
      const handles = ["n", "s", "e", "w", "nw", "ne", "se", "sw"].map(
        (pos) => {
      ...
      const ro = new win.ResizeObserver(() => {
        syncOverlay(selectedEl, outline, badge);
        try {
          if (toolbarRef.current && selectedEl) {
            positionToolbarOver(selectedEl, iframe, setToolbarPos, toolbarRef);
          }
        } catch {}
      });
      ...
      const mo = new win.MutationObserver((mutations) => {
        ...
        const sizeChanged =
          Math.abs(nr.width - prevRect.width) > 1 ||
          Math.abs(nr.height - prevRect.height) > 1;
      ...
      const beginTableResize = () => {
        ...
        selectedEl.dataset._umPrevTableLayout =
          selectedEl.style.tableLayout || "";
        ...
        if (!selectedEl.style.width || selectedEl.style.width.includes("%")) {
          selectedEl.style.width = `${Math.max(
            MIN_SIZE,
            Math.round(r.width)
          )}px`;
        }
        if (!selectedEl.style.height || selectedEl.style.height === "auto") {
          selectedEl.style.height = `${Math.max(
            MIN_SIZE,
            Math.round(r.height)
          )}px`;
        }
      ...
      const startFromPos = (pos) => (e) => {
        ...
        try {
          doc.dispatchEvent(
            new CustomEvent("umedia:resize:start", {
              bubbles: true,
              composed: true,
              detail: { el: selectedEl },
            })
          );
        } catch {}
        ...
        const end = () => {
          ...
          // שמירה מדובנסת של התוכן
          debouncedOnChange(serializeWithShortcodes(doc));
          ...
          setTimeout(() => {
            try {
              ...
              if (!d2 || !i2) return;
              const el2 = d2.querySelector(
                `[data-um-resize-marker="__dummy__"]`
              );
              if (el2) {
            ...
            try {
              doc.dispatchEvent(
                new CustomEvent("umedia:resize:end", {
                  bubbles: true,
                  composed: true,
                  detail: { el: selectedEl },
                })
      ...
      handles.forEach(({ pos, el }) => {
        ...
        el.addEventListener("touchstart", startFromPos(pos), {
          passive: false,
        });
      });
      ...
      outline.addEventListener("mousedown", (e) => {
        const p = getPoint(e);
        if (hitTest(outline, p, HIT)) edgeStart(e);
      });
      outline.addEventListener(
        "touchstart",
        (e) => {
          const t = e.touches?.[0];
          if (t && hitTest(outline, { x: t.clientX, y: t.clientY }, HIT))
            edgeStart(e);
        },
        { passive: false }
      );
  ...
  }, [selectedEl, editorRef, onContentChange, debouncedOnChange]);

  const handleDelete = () => {
    if (!selectedEl) return;

    // אם זו טבלה – נשתמש במנגנון הטבלה הקיים דרך אירוע מותאם
    if (selectedEl.tagName === "TABLE" && editorRef?.current) {
      try {
        const doc = editorRef.current.contentDocument;
        doc.dispatchEvent(
          new CustomEvent("umedia:table-action", {
            bubbles: true,
            composed: true,
            detail: { action: "deleteTable" },
          })
        );
      } catch (e) {
        console.error("table delete via event failed", e);
      }
      setSelectedEl(null);
      return;
    }

    // למדיה רגילה – מחיקה ישירה
  ...
  const handleQuickResize = () => {
    ...
    // לתמונות/וידאו/iframe – דיאלוג מידות סטנדרטי
    if (selectedEl.tagName !== "TABLE") {
      const curW =
        selectedEl.getBoundingClientRect().width ||
        Number(selectedEl.getAttribute("width")) ||
        560;
      const curH =
        selectedEl.getBoundingClientRect().height ||
        Number(selectedEl.getAttribute("height")) ||
        315;
      ...
      try {
        ...
        if (nh !== null)
          selectedEl.setAttribute("height", String(parseInt(nh, 10)));
        if (editorRef?.current) {
          const doc = editorRef.current.contentDocument;
          onContentChange?.(serializeWithShortcodes(doc));
        }
    ...
    // לטבלה – נשלח פעולה "רוחב מלא" ללוגיקת הטבלה
    if (editorRef?.current) {
      try {
        const doc = editorRef.current.contentDocument;
        doc.dispatchEvent(
          new CustomEvent("umedia:table-action", {
            bubbles: true,
            composed: true,
            detail: { action: "fullWidth" },
          })
        );
      } catch (e) {
        console.error("table full-width via event failed", e);
      }
    }
  };

  const renderToolbar = () => {
    // אין אלמנט נבחר – אין טולבר
    ...
    const style = {
      ...
      zIndex: 2147483647, // <<< מעל כל המודאלים / overlays
      display: "flex",
      gap: "4px",
      ...
      pointerEvents: "auto",
      transform: "translateZ(0)", // מונע הבזקי ציור
    };

    const fireTableAction = (action) => {
      if (!isTable || !editorRef?.current) return;
      try {
        const doc = editorRef.current.contentDocument;
        doc.dispatchEvent(
          new CustomEvent("umedia:table-action", {
        ...
        console.error("table action failed", action, e);
      }
    };

    const portalRoot = getPortalRoot(editorRef);
    if (!portalRoot) return null;

    return createPortal(
      <div
        className="umedia-toolbar"
        style={style}
        onClick={(e) => e.stopPropagation()}
        ref={toolbarRef}
      >
        {/* פעולה בסיסית: שינוי גודל / רוחב מלא לטבלה */}
        <Button
          size="icon"
          variant="ghost"
          className="size-8"
          title={isTable ? "Full width" : "Resize"}
          onClick={() => {
            if (isTable) {
              fireTableAction("fullWidth");
            } else {
              handleQuickResize();
            }
          }}
        ...
        {/* מחיקה – לכל המדיה (לטבלה דרך TableSelector) */}
        <Button
          size="icon"
          variant="ghost"
          className="size-8"
        ...
        {isTable && (
          <>
            <Button
              ...
              onClick={() => fireTableAction("openSettings")}
            >
            ...
            <Button
              ...
              onClick={() => fireTableAction("insertRowBelow")}
            >
  ...
  // שורה אחת בסוף הקומפוננטה – מחזירה את הטולבר
...
function clearAllSelections(doc) {
  doc
    .querySelectorAll(
      "img.umedia-selected, video.umedia-selected, iframe.umedia-selected, table.umedia-selected"
    )
...
/** ממיר את תוכן ה-iframe לשורטקודים של וידאו */
function serializeWithShortcodes(doc) {
  ...
  clone.querySelectorAll('div[data-wysiwyg="video"]').forEach((wrap) => {
    ...
    const sourceSrc =
      wrap.querySelector("video > source")?.getAttribute("src") || "";
    ...
    else if (/\.(ogv|ogg)(\?|#|$)/.test(lower)) attrName = "ogv";

    let w = v.getAttribute("width");
    let h = v.getAttribute("height");
    if (!w && v.style.width?.endsWith("px"))
      w = v.style.width.replace("px", "");
    if (!h && v.style.height?.endsWith("px"))
      h = v.style.height.replace("px", "");
