// src/layout/editor/components/TableEditorDialog.jsx
  ...
  useEffect(() => {
    if (selectedTable && isOpen) {
      ...
      if (firstCell) {
        ...
        setBorderColor(
          cMatch
            ? cMatch[1].trim().startsWith("rgb")
              ? rgbToHex(cMatch[1].trim())
              : cMatch[1].trim()
            : "#ddd"
        );
      ...
      if (hasZebraClass) {
        const z = window
          .getComputedStyle(selectedTable)
          .getPropertyValue("--zebra-color");
        setZebraColor(
          z && z !== "" ? rgbToHex(z) : theme === "dark" ? "#1e293b" : "#f9f9f9"
        );
      }
    }
  ...
  const applyChanges = () => {
    ...
    selectedTable.querySelectorAll("td, th").forEach((cell) => {
      ...
      if (cell.tagName === "TH" && hasHeader) {
        cell.style.backgroundColor =
          tableStructure.headerStyle.backgroundColor || "#f5f5f5";
    ...
    if (hasHeader && !existingHeader && firstRow) {
      ...
      for (let i = 1; i < selectedTable.rows.length; i++)
        tbody.appendChild(selectedTable.rows[i].cloneNode(true));

      while (selectedTable.firstChild)
      ...
      const tbody =
        selectedTable.querySelector("tbody") || document.createElement( "tbody");
        ...
        {(onClose) => (
          <>
            <ModalHeader className="flex gap-3 justify-center items-center">
              <IconBadge icon={Table2} variant="default" size="default" />
              <h2 className="text-xl font-semibold dark:text-slate-200">
                {__("Edit Table", "whizmanage")}
              </h2>
            </ModalHeader>
            <ModalBody>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* General */}
                <div>
                  <h3 className="text-lg font-medium mb-3 dark:text-slate-200">
                    {__("General", "whizmanage")}
                  </h3>
                  <div className="flex flex-col gap-3">
                    <div>
                      ...
                      <div className="flex dark:bg-slate-700 gap-0.5 mt-1">
                        ...
                        <select
                          ...
                          className="w-16 h-8 !rounded-l-none border-l-0 bg-transparent dark:!bg-slate-700 dark:!text-slate-300 rounded-md border !border-border dark:border-none text-sm"
                        >
                          <option value="%">%</option>
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      ...
                      <Label
                        ...
                        className="text-sm dark:text-slate-300"
                      >
                        {__("Header row", "whizmanage")}
                      </Label>
                    </div>
                  </div>

                  <h3 className="text-lg font-medium mt-6 mb-3 dark:text-slate-200">
                    {__("Appearance", "whizmanage")}
                  </h3>
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-4 mt-2">
                      <Switch
                        isSelected={zebra}
                        onValueChange={setZebra}
                        size="sm"
                      />
                      <Label className="text-sm dark:text-slate-300">
                        {__("Zebra rows", "whizmanage")}
                      </Label>
                    </div>
                    ...
                    {zebra && (
                      <div>
                        <Label
                          ...
                          className="text-sm dark:text-slate-300"
                        >
                          {__("Alternate row color", "whizmanage")}
                        </Label>
                        <div className="flex items-center gap-2 mt-1">
                          <input
                            ...
                            id="zebraColor"
                            value={zebraColor}
                            onChange={(e) => setZebraColor(e.target.value)}
                            className={cn(
                              "w-8 h-8 p-0 border-0 rounded",
                              `!bg-[${zebraColor}]`
                            )}
                          />
                          <Input
                  ...
                  <div className="flex flex-col gap-3">
                    <div>
                      ...
                      <div className="flex mt-1">
                        <Input
                          ...
                          value={borderWidth}
                          onChange={(e) =>
                            setBorderWidth(parseInt(e.target.value) || 0)
                          }
                          className="flex-1 h-8 !rounded-r-none dark:!text-slate-300 border dark:bg-slate-700 !border-border dark:!border-none"
                        />
                        <div className="flex items-center justify-center w-8 h-8 text-sm text-gray-500 dark:text-slate-300 border border-l-0 dark:border-none rounded-r-md dark:bg-slate-700">
                          px
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full h-8 mt-1 px-3 py-1 bg-transparent dark:!bg-slate-700 dark:!text-slate-300 rounded-md border !border-border dark:border-none text-sm"
                      >
                    ...
                    <div>
                      ...
                      <div className="flex mt-1">
                        <Input
                          ...
                          value={cellPadding}
                          onChange={(e) =>
                            setCellPadding(parseInt(e.target.value) || 0)
                          }
                          className="flex-1 h-8 !rounded-r-none dark:!text-slate-300 dark:bg-slate-700 !border-border dark:!border-none"
                        />
                        <div className="flex items-center justify-center w-8 h-8 text-sm text-gray-500 dark:text-slate-300 border border-l-0 dark:border-none rounded-r-md dark:bg-slate-700">
              ...
              <div className="border rounded p-3 mt-4 dark:border-slate-600">
                <h3 className="text-sm font-medium mb-2 dark:text-slate-300 text-center">
                  {__("Preview", "whizmanage")}
                </h3>
                <div className="overflow-auto scrollbar-whiz max-h-40 flex justify-center">
                  <table
                    style={{
                  ...
                  >
                    {hasHeader && tableStructure.rows > 0 && (
                      <thead>
                        <tr>
                          {Array.from({ length: tableStructure.cols }).map(
                            (_, j) => (
                              <th
                                key={j}
                                style={{
                                  ...
                                  padding: `${cellPadding}px`,
                                  backgroundColor:
                                    tableStructure.headerStyle
                                      .backgroundColor ||
                                    (theme === "dark" ? "#334155" : "#f5f5f5"),
                                  textAlign:
                                    tableStructure.headerStyle.textAlign ||
                                    "center",
                                  fontWeight:
                                    tableStructure.headerStyle.fontWeight ||
                                    "bold",
                                }}
                              >
