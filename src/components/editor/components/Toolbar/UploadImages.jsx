// src/layout/editor/components/Toolbar/UploadImages.jsx
...
import { Check, Copy, Trash2, Upload, Image as ImageIcon } from "lucide-react";
...
const UploadImages = ({ handleImageUpload }) => {
  ...
  const copyToClipboard = async (text, event) => {
    try {
      ...
      toast.success(__("Link copied", "whizmanage"));
    } catch (err) {
      console.error("Copy link error:", err);
      toast.error(__("Failed to copy link", "whizmanage"));
    }
  };
  ...
  const deleteImage = async (id, event) => {
    ...
    try {
      ...
      toast.success(__("The image has been deleted successfully", "whizmanage"));
      setClientImages((prev) => prev.filter((img) => img.id !== id));
    } catch (error) {
      console.error("Delete image error:", error);
      toast.error(error?.response?.data?.message || __("Failed to delete image", "whizmanage"));
    }
  };
          ...
          {(onClose) => (
            <>
              <ModalHeader className="flex gap-3 justify-center items-center">
                <IconBadge icon={ImageIcon} variant="default" size="default" />
                  ...
                  <div className="!h-[500px] w-full overflow-y-scroll">
                    ...
                    <div className="w-full h-20 flex justify-center items-center gap-2">
                      <div className="relative h-10 w-60 border rounded-lg flex gap-1 items-center px-2 dark:bg-slate-700">
                        <IoIosSearch className="w-6 h-6 text-slate-500" />
                        <Input
