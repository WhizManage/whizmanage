// src/layout/editor/components/Toolbar/ColorTools.jsx
...
const ColorTools = ({ execCommand, editorRef, focusEditor }) => {
  ...
  const checkCurrentColors = () => {
    ...
    try {
      ...
      if (
        currentTextColor &&
        !isColorSimilar(currentTextColor, defaultTextColor)
      ) {
      ...
      // צבעים שנחשבים כ"ללא רקע" - כולל צבעי רקע של הלוח במצב dark
      const transparentValues = [
        ...
        "#ffffff",
        "white",
        // צבעי רקע של הלוח במצב dark - לא נחשבים כצבע רקע שהמשתמש בחר
        "#1e293b", // slate-800
        "rgb(30, 41, 59)", // slate-800 in rgb
        "#334155", // slate-700
        "rgb(51, 65, 85)", // slate-700 in rgb
        "#0f172a", // slate-900
        "rgb(15, 23, 42)", // slate-900 in rgb
      ];
      if (
        currentBgColor &&
        !transparentValues.includes(normalizeColor(currentBgColor))
      ) {
  ...
  return (
    <ToggleGroup type="multiple" variant="ghost" size="xs" className="h-7 me-0.5">
      {/* כלי צבע טקסט */}
      <div className="relative">
        <CustomTooltip
          title={hoverTextReset ? __("Reset Text Color", "whizmanage") : __("Text Color", "whizmanage")}
        >
          <ToggleGroupItem
            value="textColor"
      ...
      <div className="relative">
        <CustomTooltip
          title={
            hoverBgReset ? __("Reset Background Color", "whizmanage") : __("Background Color", "whizmanage")
          }
        >
          <ToggleGroupItem
            value="bgColor"
