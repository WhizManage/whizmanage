...
if (! class_exists('Whizmanage_rest_functions_product')) {
	...
	class Whizmanage_rest_functions_product
	{
		...
		/**
		 * Reorder a single product relative to prev/next product (fractional indexing),
		 * then rebalance all menu_order values to integers.
		 */
		public function reorder_single_product($product_id, $prev_product_id = null, $next_product_id = null)
		{
			global $wpdb;

			// 1) Compute a temporary fractional order.
			$new_fractional_order = $this->calculate_fractional_order($prev_product_id, $next_product_id);

			// 2) Update this product with a fractional menu_order temporarily.
			$wpdb->update(
				$wpdb->posts,
				array('menu_order' => $new_fractional_order),
				array('ID' => $product_id),
				array('%f'),
				array('%d')
			);

			// 3) Normalize all products to integer menu_order.
			$updated_count = $this->rebalance_all_menu_orders();

			return array(
				'success'               => true,
				'new_fractional_order'  => $new_fractional_order,
				'updated_products'      => $updated_count,
				'message'               => 'Product reordered and all orders rebalanced',
			);
		}

		/**
		 * Compute a fractional menu_order between previous and next neighbors.
		 */
		private function calculate_fractional_order($prev_product_id, $next_product_id)
		{
			global $wpdb;
			...
			// Fractional indexing rules (keep it simple and stable).
			...
			if (! $prev_order) {
				...
				return max(1.0, $next_order - 1000.0);
			}

			if (! $next_order) {
				// Insert at the end.
				return $prev_order + 1000.0;
			}

			// Insert between both neighbors.
			return ($prev_order + $next_order) / 2.0;
		}

		/**
		 * Normalize all product menu_order values to sequential integers.
		 *
		 * This method:
		 * - Fetches all products ordered by current menu_order and ID
		 * - Reassigns menu_order to 1, 2, 3... in that order
		 * - Uses wpdb::prepare for all UPDATE queries to avoid SQL injection
		 *
		 * @return int Number of products that were re-balanced.
		 */
		private function rebalance_all_menu_orders()
		{
			global $wpdb;

			// Fetch all product IDs and their current menu_order.
			// No user input is used here, so this static query is safe.
        ...
        "
			...
			foreach ($products as $index => $product) {
				// New menu_order is just a sequential integer (1-based).
				$new_menu_order = $index + 1;

				// Use prepare() directly inside query() to avoid "NotPrepared" warnings.
				$wpdb->query(
					$wpdb->prepare(
			...
			// Clear object cache to reflect the new ordering quickly.
			wp_cache_flush();
		...
		/**
		 * REST handler: simple product reorder.
		 */
		public function handle_simple_product_reorder($request)
		{
			$product_id      = $request->get_param('product_id');
			$prev_product_id = $request->get_param('prev_product_id');
			$next_product_id = $request->get_param('next_product_id');

			$result = $this->reorder_single_product($product_id, $prev_product_id, $next_product_id);
