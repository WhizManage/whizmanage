...
function whiz_dr_table()
{
    return WHIZ_DR_TABLE;
}

function whiz_dr_now()
{
    return current_time('mysql');
}

function whiz_dr_is_admin_allowed()
{
    return current_user_can('manage_woocommerce') || current_user_can('manage_options');
}

function whiz_dr_json_decode($maybe_json)
{
 ...
 */
function whiz_dr_targets_from_rules($cond): array
{
...
// functions.php או למעלה באותו קובץ:
function whiz_dr_expand_term_ids_with_children(array $ids): array
{
    $out = [];
    foreach ($ids as $id) {
        $id = (int) $id;
 ...
 */
function whiz_dr_item_has_attribute_term(array $cart_item, string $taxonomy, int $term_id): bool {
    $term_id = (int)$term_id;
...
function whiz_dr_item_matches_any_attribute_pair(array $cart_item, array $pairs): bool {
    foreach ($pairs as $p) {
        $tax = $p['taxonomy'] ?? '';
        $tid = (int)($p['term_id'] ?? 0);
        if ($tax && $tid > 0 && whiz_dr_item_has_attribute_term($cart_item, $tax, $tid)) {
            return true;
        }
    }
    return false;
...
function whiz_dr_product_has_attribute_term(int $product_id, string $taxonomy, int $term_id): bool {
    $taxonomy = sanitize_key($taxonomy);
    if (strpos($taxonomy, 'pa_') !== 0) $taxonomy = 'pa_' . $taxonomy;
